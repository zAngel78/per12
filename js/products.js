// Solo ejecutar este código si estamos en la página de productos
if (window.location.pathname.includes('products.html')) {
    ...
    function setupFilterEvents() {
        ...
        categoryCheckboxes.forEach(checkbox => {
            checkbox.addEventListener('change', (e) => {
                ...
                loadProducts(); // Cargar productos inmediatamente al cambiar categoría
            });
        });
        ...
        if (minPriceInput && maxPriceInput) {
            minPriceInput.addEventListener('input', (e) => {
                ...
                loadProducts(); // Cargar productos al cambiar precio
            });
    ...
    // Ajustar altura de la galería en producto individual
    const adjustGalleryHeight = () => {
        const galleryMain = document.querySelector('.gallery-main');
        if (galleryMain) {
            if (window.innerWidth <= 768) {
                galleryMain.style.height = '300px';
            } else {
                galleryMain.style.height = '400px';
            }
        }
    };
